...
$wp_query = new WP_Query(); $wp_query->query('showposts=100' . '&paged='.$paged . '&post_type=faq');
$wp_query_post_first = new WP_Query(); $wp_query_post_first->query('showposts=1' . '&paged='.$paged . '&post_type=faq');
$i=0;

if(isset($_GET['post_id'])){
    $ID = $_GET['post_id'];
    $wp_query_post_first = new WP_Query();
    $wp_query_post_first->query('p='.$ID . '&post_type=faq'. '&order=DESC');
}
