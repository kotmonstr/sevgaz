...
class FM_Carousel_List_Table extends WP_List_Table {
	...
	function get_columns()
	{
		$columns = array(
				...
				'id' => __('ID', 'fm_carousel'),
				'name' => __('Name', 'fm_carousel'),
				'shortcode' => __('Shortcode', 'fm_carousel'),
				'phpcode' => __('PHP code', 'fm_carousel'),
				'time' => __('Created', 'fm_carousel')
		);
		return $columns;
	}
	...
	{
		$table_nonce = wp_create_nonce( 'fm-list-table-nonce' );
		...
		switch( $column_name ) {
			case 'id':
				...
				else
				{
					$actions = array(
							...
							'view' => sprintf('<a href="?page=%s&itemid=%s">View</a>', 'fm_carousel_show_item', $item['id']),
							'edit' => sprintf('<a href="?page=%s&itemid=%s">Edit</a>', 'fm_carousel_edit_item', $item['id'])
					);
				}
			...
			case 'shortcode':
				return esc_attr('[fm_carousel id="' . $item['id'] . '"]');
			case 'phpcode':
				return esc_attr('<?php echo do_shortcode(\'[fm_carousel id="' . $item['id'] . '"]\'); ?>');
			default:
				return $item[ $column_name ];
		}
	}
	...
	function get_views(){
		...
		// All
		$all_url = admin_url('admin.php?page=fm_carousel_show_items');
		$class = ($current == 'all' ? ' class="current"' :'');
		$views['all'] = "<a href='" . $all_url . "' " . $class . " >All (" . $this->published . ")</a>";
	
		// Published
		$publish_url = admin_url('admin.php?page=fm_carousel_show_items&item_status=publish');
		$class = ($current == 'publish' ? ' class="current"' :'');
		$views['publish'] = "<a href='" . $publish_url . "' " . $class . " >Published (" . $this->published . ")</a>";
	
		// Trash
		$trash_url = admin_url('admin.php?page=fm_carousel_show_items&item_status=trash');
