...
class FM_Carousel_Update {
	...
	function __construct($controller) {
		...
		$this->slug = 'fm-carousel';
		$this->plugin = FM_CAROUSEL_PLUGIN;
		$this->version = FM_CAROUSEL_PLUGIN_VERSION;
		$this->wp_version = $wp_version;
		$this->domain = $_SERVER['SERVER_NAME'];
		$this->api_url = 'https://www.remove.me.please/update-check.php';
	...
	function plugin_update_notice() {
		...
		if( $pagenow === 'update-core.php')
		{
			if ( !isset($info->key_status) || $info->key_status != 'valid')
			{
				...
				{
					echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: Your free upgrade period has expired. To update the plugin, please <a href="https://www.remove.me.please/renew/" target="_blank">renew your license</a>, otherwise, you will get the <strong>Update package not available</strong> error.</p></div>';
				}
				else
				{
					if (FM_CAROUSEL_VERSION_TYPE == 'F')
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin in WordPress backend, please <a href="https://www.remove.me.please/order/?product=carousel" target="_blank">upgrade to a commercial license</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';				
					}
					else
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin, please <a href="'. admin_url('admin.php?page=fm_carousel_register') . '">enter your license key</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';
					}
				}
			}
		}
	...
	function show_plugin_update_message( $plugin_data, $r ) {
		...
		{
			echo '<br>Your free upgrade period has expired. To get automatic update, please <a href="https://www.freemake.com/renew/" target="_blank">renew your license</a>.';
		}
		else
		{
			if (FM_CAROUSEL_VERSION_TYPE == 'F')
			{
				echo '<br>To get automatic update, please <a href="https://www.remove.me.please/order/?product=carousel" target="_blank">upgrade to a commercial license</a>.';
			}
			else
			{
				echo '<br>To get automatic update, please <a href="'. admin_url('admin.php?page=fm_carousel_register') . '">enter your license key</a> or <a href="https://www.remove.me.please/register-faq/#manualupdate" target="_blank">update the plugin manually</a>.';
			}
		}
	}
}